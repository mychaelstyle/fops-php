...
class Photo implements Ops {
  ...
  public function canDo($path){
    ...
    $ext  = array_pop($elms);
    if('jpg'==strtolower($ext) || 'cr2'==strtolower($ext)){
      return true;
    }
    return false;
  }
  ...
  public function ooops($filePath,Printers $printers){
    $this->read($filePath);
    $printers->output($filePath);
    echo $filePath."\n";
    echo $this->getRegulatedPath('/Users/masanori/photos/')."\n\n";
    ob_flush();
  }
